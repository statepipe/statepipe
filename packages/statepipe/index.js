import {validateNode, validateState, QUERY_STATEPIPE, ACTION_ATTR, STATE_ATTR, queryComponents, getStatepipeName, warn} from "../common";
...
const statepipe = (wrapper, stores) => {
  ...
  if (!validateState(stores)
  || !validateNode(wrapper)) {
    warn(":statepipe invalid store/wrapper");
    return null;
  }

  let children = Array
    .from(wrapper.querySelectorAll(QUERY_STATEPIPE)||[]);
