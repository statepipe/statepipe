...
// a different context
const myContext = ctx => node =>
  Array.from(ctx.wrapper.querySelectorAll(utils.QUERY_STATEPIPE) || []).reduce(
    (mine, newwrapper) => {
      if (mine === true && newwrapper.contains(node)) {
...
const handleMutation = ctx => {
  ...
  return mutations =>
    Array.from((mutations = mutations || [])).forEach(mutation => {
      if (mutation.removedNodes.length) {
        const list = Array.from(mutation.removedNodes).filter(
          utils.validateNode,
        );
      ...
      if (mutation.addedNodes.length) {
        const list = Array.from(mutation.addedNodes).filter(utils.validateNode);

        const newSchemas = list
          .filter(node => {
            if (utils.validateProp(node.getAttribute(utils.STATEPIPE_ATTR))) {
              //new context here
        ...
        if (newSchemas.length) {
          ctx.components = ctx.components.concat(newSchemas);
        }
      }

      const isAction = !!(mutation.attributeName || '').match(
        utils.ACTION_ATTR,
      );
      ...
      const action = isAction
        ? mutation.target.getAttribute(utils.ACTION_ATTR)
        : null;
      const payload = isAction
        ? utils.parseJSON(mutation.target, utils.PAYLOAD_ATTR)
        : null;
