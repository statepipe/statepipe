...
const getPayload = (schema, event) => (acc, reducer) => {
  ...
  typeof reducer.run === "function") {
    const prefix = `:statepipe ${schema.wrapper.getAttribute(":statepipe")}: ${schema.type}/ ${schema.node.nodeName}`;
    try {
  ...
  return event => {
    ...
    const resolveTrigger = (block, value) => {
      if (utils.validateState(value) && Object.keys(value).length) {
        utils.log(`:statepipe ${schema.wrapper.getAttribute(":statepipe")}: ${schema.type}/ ${schema.node.nodeName} (${block.index}) ∴ action=${block.action} payload`, value);
