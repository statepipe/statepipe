...
test('propPick/ context args: event, node, wrapper' , t => {
  const state = {a:true}
  ...
  t.deepEqual(merge(state,tester), dom.propPick("self","blur","a.b")({state, node}))
  t.deepEqual(merge(state,tester), dom.propPick("wrapper","a.b","blur")({state, wrapper}))
})
...
test('propSet/ context args: event, node, wrapper' , t => {
  ...
  dom.propSet("self","a.b","a.b")({state, node})
  t.deepEqual(state.a.b, node.a.b)
  dom.propSet("wrapper","a.b","a.b")({state, wrapper})
  t.deepEqual(state.a.b, wrapper.a.b)
})
