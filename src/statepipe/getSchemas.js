...
export default stores => {
  ...
  return (item) => {
    ...
    const slugList = (
      item.node
        .getAttribute(utils.ALIAS_ATTR[item.type]) || "")
        .replace(/\s|\r|\r\n|\n|\s/gmi,"")
        .trim()
        .split(",");
    ...
    if (global.$statepipeLog){
      result.reducers.forEach(o => {
        utils.log(`:statepipe ${item.node.statepipe}: ${item.type}/ ${item.node.nodeName} ⋆ ${o.fn} args:${o.args.join(",")}`);
      });
    }
